 ...
 * @version 1.0.1
 */
public class FileDrop implements MouseListener {
	...
	public void listFilesForFolder(final File folder, ArrayList<File> list, String[] filter) {
		for (File fileEntry : folder.listFiles()) {
			fileEntry = new File(fileEntry.getPath().replace("./", ""));
			if (fileEntry.isDirectory()) {
				
				listFilesForFolder(fileEntry, list,filter);
			} else {
				if(accept(fileEntry,filter)){
				list.add(fileEntry);
				}
			}		
		}
	...
	private boolean accept(File fileEntry, String[] filter) {
		
		for (String ext : filter) {
			if(fileEntry.getAbsoluteFile().toString().toLowerCase().endsWith(ext.toLowerCase())) return true;
		}
		return false;
	}

	private File[] getAllFilesArray(final File folder, String[] filter) {
		ArrayList<File> list = new ArrayList<File>();
		listFilesForFolder(folder, list,filter);

		File[] arrayList = new File[list.size()];
