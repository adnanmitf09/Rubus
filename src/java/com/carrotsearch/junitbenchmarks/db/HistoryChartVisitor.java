        ...
        for (Map.Entry<Class<?>, List<Method>> e : types.entrySet())
        {
            ...
            if (ann != null)
            {
                HistoryChartGenerator gen = new HistoryChartGenerator(
                    ...
                    clazz.getName(), 
                    ann.labelWith(),
                    c,ann.timeUnit(),ann.customLabel(),ann.showClassName());
            ...
            for (Map.Entry<String, List<Method>> e2 : byPrefix.entrySet())
            {
                ...
                HistoryChartGenerator gen = new HistoryChartGenerator(
                    ...
                    c, e2.getValue().get(0).getAnnotation(BenchmarkHistoryChart.class).timeUnit(),e2.getValue().get(0).getAnnotation(BenchmarkHistoryChart.class).customLabel(),
                    e2.getValue().get(0).getAnnotation(BenchmarkHistoryChart.class).showClassName());
